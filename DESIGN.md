...
## 0.8 additions - saved transcripts
- **SAVED** is a footer label-button beside `LOG` and opens a view in the same slot (chain strip and footer stay; `✕ BACK TO STAGE` and Esc return). Sub-bar: `SAVED TRANSCRIPTS` · `N SAVED` · the saving state, then `OPEN FOLDER` as a dim link. The state reads `NOT SAVING` in amber when the last line could not be written, with the reason on hover.
- **Two panes**: sessions on the left, 220-280px, one row each - date and time, then mono meta `42 LINES · 18 MIN · 12 KB`, with `· RECORDING` in amber on the session still being written; the open row carries a 2px amber inset rule. The reader fills the right. Below 760px the panes stack.
- **Reader**: the stage's own caption rows - timestamp gutter, speaker tag, original and translation side by side - with the gutter reading time into the session (`00:00:02`), not wall clock. A session with no translation collapses to one column. Header: the session's date, then mono actions `EXPORT .TXT · EXPORT .SRT · SHOW IN FOLDER · DELETE`. DELETE arms to `SURE?` for five seconds, the same shape as `NEW`, and is mute for the session still recording.
- The newest session is open in the reader on arrival - after a crash, that is the one wanted.
- **SETTINGS → THIS APP → TRANSCRIPTS**: field status (`ON` / `SAVING` / `NOT SAVING` in amber / `OFF`), a square toggle `SAVE TRANSCRIPTS`, the folder in mono `--ink-2`, and mono actions `CHANGE FOLDER · USE DOCUMENTS · OPEN SAVED` - `USE DOCUMENTS` only once another folder has been chosen.
- No new colour: amber marks the failure state, `RECORDING`, and the open row.
